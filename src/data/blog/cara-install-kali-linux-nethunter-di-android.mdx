...
/>
--- ## Mengubah Bahasa ke Indonesia - Buka pengaturan bawaan NetHunter di Termux menggunakan:
<Pre
  codeBlocks={[
    {
...
/>
> Password default `sudo` NetHunter adalah: `kali`. - Konfirmasi pilihan bahasa:
<Pre
  codeBlocks={[
    {
...
/>
- Cari `id_ID.UTF-8 UTF-8`, pilih menggunakan `Spasi`, lalu `Enter`. --- ## Install GUI
<Pre
  codeBlocks={[
    {
...
/>
> Perhatikan terminal Termux karena akan muncul beberapa prompt yang wajib dikonfirmasi! --- ## Akses Kali Linux - Masukkan
perintah `kex`, lalu buka AVNC, atur seperti berikut:
<Pre
  codeBlocks={[
    {
...
/>
- Isi `Password` sesuai dengan yang sudah kamu buat. --- ## Memperbaiki Audio dan Mic - Unduh dan install terlebih dahulu
[Termux:API](https://github.com/termux/termux-api/releases/) dan pastikan kamu sudah menginstal `pulseaudio` di dalam Termux
maupun NetHunter, lalu masukkan perintah berikut di Termux:
<Pre
  codeBlocks={[
    {
...
/>
> Kamu dapat memasukkan kedua perintah di atas ke dalam `.bashrc` ataupun `.zshrc`. --- > Untuk menginstal Node, silahkan
baca [Instal NVM (Node Version Manager)](/posts/cara-install-ubuntu-dan-node-di-android-menggunakan-termux#instal-nvm-node-version-manager).
---
